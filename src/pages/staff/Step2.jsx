...
export default function Step2() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (inputTons > selectedLot.receivedTons) {
      setMessage({ type: 'error', text: `ตัดออกไม่เกิน ${selectedLot.receivedTons} ตัน` });
      return;
    }
    setLoading(true);
    try {
      ...
      await addDoc(collection(db, 'processing'), {
        ...
        lotTagCode: selectedLot.tagCode || '',
        tagCode: formData.tagCode,
        round: roundNumber,
        originalVariety: selectedLot.riceVariety,
        newVariety: formData.newVariety,
        germinationTestDate: formData.germinationTestDate,
        sortingDate: formData.sortingDate,
        tagIssueDate: formData.tagIssueDate,
        inputTons,
        outputBags,
        ...
        tagHistory: [{
          tagCode: formData.tagCode,
          variety: formData.newVariety,
          changedBy: user.name,
          changedAt: now,
          note: `คัดรอบที่ ${roundNumber}`
        }],
        varietyHistory: varietyChanged ? [{
          from: selectedLot.riceVariety,
          to: formData.newVariety,
          changedBy: user.name,
          changedAt: now,
        }] : [],
      });
      ...
      // ✅ อัพเดท summary
      await updateVarietySummary(formData.newVariety, {
        deltaBags: outputBags,
        deltaTons: inputTons,
      });
    ...
    try {
      ...
      if (editForm.tagCode !== editingProc.tagCode) {
        ...
        tagHistory.push({
          ...
          oldTagCode: editingProc.tagCode,
          changedBy: user.name,
          changedAt: now,
      ...
      if (editForm.newVariety !== editingProc.newVariety) {
        ...
        varietyHistory.push({
          from: editingProc.newVariety,
          to: editForm.newVariety,
          changedBy: user.name,
          changedAt: now,
          note: editForm.editNote || 'แก้ไขสายพันธุ์'
        });
        // ย้าย bags ใน summary
        const bags = editingProc.outputBags || 0;
        await updateVarietySummary(editingProc.newVariety, {
          deltaBags: -bags,
          deltaTons: -(editingProc.inputTons || 0)
        });
        await updateVarietySummary(editForm.newVariety, {
          deltaBags: bags,
          deltaTons: editingProc.inputTons || 0
        });
      }
      ...
      await updateDoc(doc(db, 'processing', editingProc.id), {
        tagCode: editForm.tagCode,
        newVariety: editForm.newVariety,
        germinationTestDate: editForm.germinationTestDate,
        sortingDate: editForm.sortingDate,
      ...
      await addDoc(collection(db, 'activityLog'), {
        ...
        lotCode: editingProc.lotCode,
        round: editingProc.round,
        changes,
    ...
    <div className="pt-6 pb-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        🏷️ <span className={`font-bold ${lot.tagCode ? 'text-blue-700' : 'text-red-400'}`}>
                          {lot.tagCode || 'ยังไม่ได้ใส่ใบแท็ก'}
                        </span>
                      </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="text" value={formData.tagCode}
                ...
                className="w-full border-2 border-yellow-300 rounded-xl p-3 focus:outline-none focus:border-yellow-500 font-bold text-blue-700" />
            </div>

            <div>
              <label className="text-sm text-gray-700 font-bold block mb-1">🌾 พันธุ์ข้าว (หลังคัด) *</label>
              <div className="flex gap-2">
                <select value={formData.newVariety}
                  onChange={e => setFormData({...formData, newVariety: e.target.value})}
              ...
              {showAddVariety && (
                <div className="mt-3 flex gap-2 bg-blue-50 p-3 rounded-xl border-2 border-blue-200">
                  <input type="text" value={newVarietyName}
                    onChange={e => setNewVarietyName(e.target.value)}
                    placeholder="ชื่อพันธุ์ข้าวใหม่"
                    className="flex-1 border-2 border-blue-300 rounded-lg p-2 text-sm" />
              ...
              {formData.newVariety && formData.newVariety !== selectedLot.riceVariety && (
                <p className="text-xs text-orange-500 mt-1">
                  ⚠️ เปลี่ยนจาก "{selectedLot.riceVariety}" → "{formData.newVariety}" (จะบันทึกประวัติไว้)
                </p>
              )}
            </div>
            ...
            <div>
              <label className="text-sm text-gray-700 font-bold block mb-1">
                ⚖️ จำนวนตันที่ตัดออก (คงเหลือ: {selectedLot.receivedTons} ตัน) *
              </label>
              <input type="number" step="0.1" max={selectedLot.receivedTons}
                value={formData.inputTons}
                onChange={e => setFormData({...formData, inputTons: e.target.value})}
                placeholder={`สูงสุด ${selectedLot.receivedTons} ตัน`}
                className="w-full border-2 border-gray-200 rounded-xl p-3 focus:outline-none focus:border-green-500" />
              {formData.inputTons && (
                <p className="text-xs text-gray-400 mt-1">
                  จะเหลือ: {(selectedLot.receivedTons - parseFloat(formData.inputTons || 0)).toFixed(2)} ตัน
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <input type="number" value={formData.outputBags}
                onChange={e => setFormData({...formData, outputBags: e.target.value})}
                placeholder="เช่น 150"
                className="w-full border-2 border-gray-200 rounded-xl p-3 focus:outline-none focus:border-green-500" />
            </div>

            <div>
              <label className="text-sm text-gray-700 font-bold block mb-1">📅 วันที่ทดสอบการงอก</label>
              <input type="date" value={formData.germinationTestDate}
                onChange={e => setFormData({...formData, germinationTestDate: e.target.value})}
                className="w-full border-2 border-gray-200 rounded-xl p-3 focus:outline-none focus:border-green-500" />
            </div>

            <div>
              <label className="text-sm text-gray-700 font-bold block mb-1">📅 วันที่คัดข้าว</label>
              <input type="date" value={formData.sortingDate}
                onChange={e => setFormData({...formData, sortingDate: e.target.value})}
                className="w-full border-2 border-gray-200 rounded-xl p-3 focus:outline-none focus:border-green-500" />
            </div>

            <div>
              <label className="text-sm text-gray-700 font-bold block mb-1">📅 วันที่ออกแท็ก</label>
              <input type="date" value={formData.tagIssueDate}
                onChange={e => setFormData({...formData, tagIssueDate: e.target.value})}
                className="w-full border-2 border-gray-200 rounded-xl p-3 focus:outline-none focus:border-green-500" />
            </div>
      ...
      <div className="bg-white rounded-2xl shadow p-5">
        <button onClick={() => setShowHistory(!showHistory)}
          ...
          <span>📋 รายการที่คัดแล้ว ({processingList.length})</span>
              ...
              <div key={proc.id} className="border border-gray-200 rounded-xl p-4">
                {editingProc?.id === proc.id ? (
                  <div className="space-y-3">
                    ...
                    <div>
                      <label className="text-xs text-gray-500 font-bold">🌾 พันธุ์ข้าว (หลังคัด)</label>
                      <select value={editForm.newVariety}
                        onChange={e => setEditForm({...editForm, newVariety: e.target.value})}
                        className="w-full border-2 border-gray-200 rounded-lg p-2 mt-1">
                        <option value="">-- เลือกพันธุ์ --</option>
                        {varieties.map(v => <option key={v}>{v}</option>)}
                      </select>
                      {editForm.newVariety !== proc.originalVariety && (
                        <p className="text-xs text-orange-500 mt-1">⚠️ ต้นทาง: {proc.originalVariety}</p>
                      )}
                    </div>

                    <div>
                      <label className="text-xs text-gray-500 font-bold">📦 จำนวนกระสอบ</label>
                      <input type="number" value={editForm.outputBags}
                        onChange={e => setEditForm({...editForm, outputBags: e.target.value})}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <input type="date" value={editForm.germinationTestDate}
                          onChange={e => setEditForm({...editForm, germinationTestDate: e.target.value})}
                          className="w-full border-2 border-gray-200 rounded-lg p-2 mt-1 text-sm" />
                      </div>
                      <div>
                        <label className="text-xs text-gray-500 font-bold">📅 วันคัดข้าว</label>
                        <input type="date" value={editForm.sortingDate}
                          onChange={e => setEditForm({...editForm, sortingDate: e.target.value})}
                          className="w-full border-2 border-gray-200 rounded-lg p-2 mt-1 text-sm" />
                      </div>
                    </div>
                    ...
                    {(proc.tagHistory?.length > 0 || proc.varietyHistory?.length > 0) && (
                      <div className="bg-gray-50 rounded-lg p-3">
                        <p className="text-xs font-bold text-gray-500 mb-1">📜 ประวัติการแก้ไข</p>
                        {proc.tagHistory?.map((h, i) => (
                          <p key={i} className="text-xs text-gray-500">
                            🏷️ {h.oldTagCode || '-'} → {h.tagCode} ({h.changedBy})
                          </p>
                        ))}
                        {proc.varietyHistory?.map((h, i) => (
                          <p key={i} className="text-xs text-gray-500">
                            🌾 {h.from} → {h.to} ({h.changedBy})
                          </p>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        🌾 {proc.newVariety}
                        {proc.newVariety !== proc.originalVariety && (
                          <span className="text-xs text-orange-500 ml-1">(จาก {proc.originalVariety})</span>
                        )}
                      </p>
                      <p className="text-sm text-gray-500">
                        ⚖️ {proc.inputTons} ตัน → 📦 {proc.outputBags || '-'} กระสอบ
                      </p>
                      <p className="text-sm text-gray-500">📅 {proc.sortingDate || '-'}</p>
                    </div>
                    <button onClick={() => {
                      setEditingProc(proc);
                      setEditForm({
                        tagCode: proc.tagCode || '',
                        newVariety: proc.newVariety || '',
                        germinationTestDate: proc.germinationTestDate || '',
                        sortingDate: proc.sortingDate || '',
                        tagIssueDate: proc.tagIssueDate || '',
